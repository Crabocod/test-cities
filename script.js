...
function apply() {
	...
	var uppUserCity = userCity.charAt(0).toUpperCase() + userCity.slice(1);
	if (uppUserCity == "азов" || uppUserCity == "Азов") {
		alert("Такой город уже был!");
		return false;
	}
		if (newCities.indexOf(uppUserCity) == -1) {
			alert("Такого города нет или это не Российский город");
			return false
		}

	if (!(outCity[outCity.length-1] == uppUserCity[0].toLowerCase() || outCity[outCity.length-2] == uppUserCity[0].toLowerCase()) ) {
		alert("Вы ввели город не на ту букву!");
		return false;
	}

	for (var i = 0; i < usedCities.length; i++) {
		if(usedCities[i] == uppUserCity){
					alert("Такой город уже был!");
					return false;
			}
	}
	...
	if (uppUserCity == newCities[i]) {
		newCities.splice(i,1);
		for (var i = 0; i < newCities.length; i++) {

			if(newCities[i][0].toLowerCase() == uppUserCity[uppUserCity.length-1]){
			outCity = newCities[i];
			alert(newCities[i]);
			usedCities.push(newCities.splice(i,1));
			usedCities.push(uppUserCity);
	...
	if (uppUserCity == newCities[i]) {
		...
		for (var i = 0; i < newCities.length; i++) {
			...
			if(newCities[i][0].toLowerCase() == uppUserCity[uppUserCity.length-2]){
			outCity = newCities[i];
			alert(newCities[i]);
			usedCities.push(newCities.splice(i,1));
			usedCities.push(uppUserCity);
